...
const api: GriffinApi = {
  ...
  saveBooklet: (booklet, options) => ipcRenderer.invoke('booklet:save', booklet, options),
  openBooklet: () => ipcRenderer.invoke('booklet:open'),
  ...
  onUpdateState: (handler) => {
    const listener = (_event: Electron.IpcRendererEvent, info: unknown) => handler(info as Parameters<typeof handler>[0]);
